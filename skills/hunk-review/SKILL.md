...
If no session exists, ask the user to relaunch pi with the `--hunk` flag, then retry.

In this sandboxed environment, always pass `--repo "$HOST_PWD"` (the host checkout path) to session commands instead of `--repo .`, unless the user specifies a different repo.
...
7. hunk session reload -- <command>                     # swap contents only for non-PR sessions
...
When working in a specific repo (e.g. narcore), only inspect/act on the Hunk session whose `repoRoot`/`Repo` matches that repo. If `hunk session list` shows sessions for other repos (e.g. NarGateway), ignore them entirely -- don't summarize or act on their comments unless the user explicitly asks about that other repo.
...
> **PR comment safety — permanent rule:** Never run `hunk session reload` on a session containing imported PR review comments. Reload replaces the review source and clears imported human comments and existing local replies from the live session (the remote PR remains intact, but the local review context is lost). Treat any human-authored comment carrying `threadId:` in its rationale as proof that the session is PR-imported. Use `review`, `context`, navigation, and `comment list` without reloading. If the PR view must be refreshed, ask the user to relaunch pi with `--hunk` so comments are re-imported safely.

For sessions without imported PR comments, reload swaps the live session's contents. Pass a Hunk review command after `--`:
...
- Before every reload, run `comment list --type all --json`; abort the reload if any imported human PR comment is present
- Never reload merely to refresh changed working-tree files while addressing PR comments
...
- For non-PR sessions only, if the live session is already showing the target worktree, prefer `hunk session reload --repo /path/to/worktree -- diff`
...
### Replying to imported review comments

Imported review comments carry a thread identifier in their `rationale` field. When replying to one:

1. Run `hunk session comment list --repo $HOST_PWD --json` and find the target comment's `rationale`, e.g. `threadId: <N>`.
2. Add your reply with the **same** `threadId: <N>` in `--rationale`, and the actual reply text in `--summary`.
3. **Always pass `--author agent` on the reply.** This is not optional -- omitting it is the most common mistake when following this workflow. Do not reuse the original comment's author.

```bash
hunk session comment add --repo $HOST_PWD --file README.md --new-line 103 \
  --author agent --rationale "threadId: 42" --summary "Actual reply text here"
```

- Match replies by `threadId` from `rationale`, not by file/line -- your reply may land on a different line than the original comment if the code has shifted.
- Before running the command, double-check it includes `--author agent` literally in the argument list.

## Multi-pass review workflow

When review comments/fixes span multiple agent sessions before a push (fix + reply across
passes), follow `multi-pass-workflow.md` (same directory) — it covers pass structure, one-reply-
per-thread discipline, and delete-and-recreate semantics when a later pass supersedes an earlier
reply.

## New files in working-tree reviews

`hunk diff` includes untracked files by default. In a non-PR session, if the user wants tracked changes only, reload with `--exclude-untracked`. Never use this reload in a session with imported PR comments:
...
1. Load the right content (`reload` only for sessions with no imported PR comments; otherwise preserve the current source)
...
- **"No active Hunk sessions"** -- ask the user to relaunch pi with the `--hunk` flag, then retry. If pi was already launched with `--hunk` and Hunk is visibly running, localhost may be blocked by the agent sandbox; retry with network/sandbox escalation.
